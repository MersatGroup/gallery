...
const Gallery = () => {
    ...
    const { data: albums, isLoading, error} = useFetch('http://localhost:3001/album/');
    const classes = useStyles();

    return (
        <>
        <Grid container display="flex" justify="flex-end" className={classes.marginY1}>
            <Grid item lg={2} >
        ...
        <Grid container spacing={3} display="flex" className={classes.paddingX1}>
            ...
            {albums && <ListAlbums albums={albums} albumId={albums.id}>                    
                </ListAlbums>
            }
        </Grid>
        </>
